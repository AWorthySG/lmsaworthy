...
function FormulaCards() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        <h1 style={{ fontSize: 28, fontWeight: 800, background: T.gradPrimary, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", margin: 0, fontFamily: T.fontDisplay }}>Formula Cards</h1>
      ...
      {Object.entries(FORMULA_CARDS).map(([subj, cards]) => {
        ...
        return (
          <div key={subj}>
            <div style={{ fontSize: 16, fontWeight: 800, color: theme.accent, marginBottom: 10, fontFamily: T.fontDisplay }}>{subjName}</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 10 }}>
              {cards.map((c, i) => (
                <div key={i} className="card-enter" style={{ "--i": i, background: T.bgCard, borderRadius: T.r2, padding: "16px", border: `1px solid ${T.border}`, borderTop: `3px solid ${theme.accent}` }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 800, color: T.text, fontFamily: T.fontMono, marginBottom: 6, lineHeight: 1.4 }}>{c.formula}</div>
