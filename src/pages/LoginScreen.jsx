...
function LoginScreen() {
  ...
  const inputStyle = {
    ...
    border: "1.5px solid rgba(254,254,254,0.1)", background: "rgba(254,254,254,0.05)",
    fontSize: 14, boxSizing: "border-box", color: "#FEFEFE",
  ...
  return (
    <div className="login-root">
      <style>{`
        ...
        .login-brand { max-width: 480px; width: 100%; animation: loginFadeUp 0.6s ease both; }
        .login-form-wrap {
        ...
        @media (min-width: 768px) {
          ...
          .login-brand { max-width: 440px; }
          .login-form-wrap { max-width: 400px; }
        }

        /* ── Desktop / TV (≥1200) ── */
        @media (min-width: 1200px) {
          .login-layout { padding: 60px 56px; gap: 80px; }
          .login-brand { max-width: 520px; }
          .login-form-wrap { max-width: 440px; }
          .login-hero { font-size: 52px !important; }
        }

        /* ── Large TV (≥1800) ── */
        @media (min-width: 1800px) {
          .login-layout { padding: 80px; gap: 120px; }
          .login-brand { max-width: 640px; }
          .login-form-wrap { max-width: 480px; }
          .login-hero { font-size: 64px !important; }
        }

        /* ── Feature grid: 1-col on very small, 2-col otherwise ── */
        .login-features { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
        @media (max-width: 360px) { .login-features { grid-template-columns: 1fr; } }

        /* ── Stats row: wrap on tiny screens ── */
        .login-stats { display: flex; flex-wrap: wrap; gap: 24px; margin-bottom: 32px; }
        @media (min-width: 768px) { .login-stats { gap: 32px; margin-bottom: 44px; } }

        /* ── Form card ── */
        .login-card {
          width: 100%; background: rgba(254,254,254,0.04); border-radius: 20;
          padding: 28px 24px; border: 1px solid rgba(254,254,254,0.09);
          backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px);
          box-shadow: 0 24px 64px rgba(0,0,0,0.4); border-radius: 20px;
        }
        @media (min-width: 768px) { .login-card { padding: 36px 32px; } }
        ...
        @media (min-width: 768px) { .login-orb { display: block; } }
      `}</style>
            ...
            <form onSubmit={mode === "login" ? handleLogin : handleRegister}>
              {mode === "register" && (
                <div style={{ marginBottom: 14 }}>
                  ...
                  <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Sarah Chen" className="login-input" style={inputStyle} />
                </div>
              )}
              <div style={{ marginBottom: 14 }}>
                <label style={labelStyle}>Email Address</label>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" required className="login-input" style={inputStyle} />
              </div>
