...
- `initialState` is rehydrated from `localStorage` key `aworthy-lms-state` on load. Only keys listed in `PERSIST_KEYS` are persisted: `bookmarks`, `attendance`, `submissions`, `homework`, `pastPaperDocs`, `studyLogs`, `notes`, `announcement`, `goals`, `mistakes`, `revisionChecklist`, `posts`, `reports`, `sessions`, `videoLessons`. Everything else re-seeds from `src/data/seedData.js` on every load. (`ratings` has been removed from both `DEFAULT_STATE` and `PERSIST_KEYS`.)
...
- `src/icons/icons.jsx` — Phosphor icon wrappers used throughout. All UI indicators use these icons, not raw emojis. Add new icons here rather than importing `@phosphor-icons/react` directly in pages. Current exports include `SortAscending`, `CaretLeft`, `Trash`, `Check`, `Paperclip`, `Smiley`, `Leaf`, `MusicNote`.
...
- `src/utils/` — `helpers.js` (subject themes, exam countdowns, daily challenges, word of the day, study plan generation), `notifications.js` (homework reminders — all `Notification` references are guarded with `typeof Notification !== "undefined"` because iOS Safari does not define `Notification` as a global; bare references crash the entire app), `spacedRepetition.js` (spaced repetition scheduling for revision).
...
### PWA / Capacitor
- `public/sw.js` (cache name `aworthy-lms-v8`, separate `aworthy-assets-v8` for hashed bundles) — cache-first for `/assets/*` (immutable hashed filenames), network-first with 4s timeout for navigation (falls back to cached `/index.html`), network-first with 5s timeout for everything else. **Bump both cache names whenever you change shell assets or deploy significant JS changes** — this forces all cached devices to evict stale bundles.
